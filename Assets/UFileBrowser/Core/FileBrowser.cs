...
public class FileBrowser : UIBehaviour {
	...
	public void Submit () {
            ...
            if (SelectingID <= 0)
            {
                m_OnSubmit.Invoke(AddressPath);
                gameObject.SetActive(false);
            }
            else if (m_OnlySubmitExistPath) {
	...
	public void ReloadBookMark () {
		...
		for (int i = 0; i < len; i++) {
			...
			g.SetActive(true);
			Transform tf = g.transform.FindChild("Text");
			if (tf) {
	...
	private void AddItem (int id, bool isFile, bool selecting) {
		if (!ItemMap.ContainsKey(id)) {
			...
			if (info != null) {
				...
				Transform iconTF = g.transform.FindChild("Icon");
				if (iconTF) {
				...
				Transform textTF = g.transform.FindChild("Text");
				if (textTF) {
